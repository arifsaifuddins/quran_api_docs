...
function InputField() {
  ...
  const Commit = () => {
    ...
    setSable(false)
    setError('Data Commited, Now Push!')
    setErrorAd(true)
  }
  ...
  return (
    <div className="flex flex-col mx-auto mb-16 w-[700px]">
      {
        (ErrorAd == true) && <div className="text-xl bg-slate-500 py-2 px-3 font-bold mb-4 rounded-lg text-white flex justify-between items-center">
          <div>{Error}</div>
          <button onClick={() => setErrorAd(false)} className="text-slate-600 hover:text-red-700 text-2xl font-bold">&times;</button>
        </div>
      }
      <div className="flex justify-between items-center">
        <div className="flex">
          <input type="number" max="286" min="1" autoFocus placeholder="Number of Ayah" className="mr-2 text-white w-56 bg-transparent py-2 px-3 rounded-lg text-lg  border outline-none border-slate-400" onChange={(e) => setAyah(e.target.value)} />
          <input type="number" max="144" min="1" placeholder="Number of Surah" className="ml-2 text-white w-56 bg-transparent py-2 px-3 rounded-lg text-lg  border outline-none border-slate-400" onChange={(e) => setSurah(e.target.value)} />
        </div>
        <div className="flex">
          <button onClick={() => Commit()} className=" bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-5 rounded-lg text-lg mr-2" >Commit</button>
          {
            (Sable == true) ? <button disabled onClick={() => Push()} className="bg-green-700 cursor-not-allowed font-bold py-2 px-5 rounded-lg text-lg" >Push</button> : <button onClick={() => Push()} className=" bg-green-500 hover:bg-green-700 text-white font-bold py-2 px-5 rounded-lg text-lg" >Push</button>
          }
